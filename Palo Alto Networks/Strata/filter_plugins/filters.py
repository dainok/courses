...
class FilterModule(object):
    ...
    def panos_content(self, output: dict, latest: bool = False) -> str:
        ...
        try:
            ...
            for entry in entries:
                ...
                if float(latest_version.replace("-", ".")) < float(entry["version"].replace("-", ".")):
    ...
    def panos_content_period(self, output: dict, hours: int = 86400) -> bool:
        ...
        try:
            data = json.loads(output)
            last_updated_at = data["response"]["result"]["content-updates"]["@last-updated-at"]
            if not last_updated_at:
                raise ValueError("string is not output from request content upgrade info")
            delta = (datetime.now(pytz.utc) - dateparser.parse(last_updated_at)).total_seconds()
            if delta < hours * 3600:
                return True
            return False
        except json.decoder.JSONDecodeError:
